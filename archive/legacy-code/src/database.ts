// @crumb database-persistence
// DAT | supabase_initialization | upsert_operations | batch_queries | exclusion_syncing | profile_lookup
// why: Persist validated profiles to Supabase; provide graceful fallback when DB unavailable (local-only mode)
// in:env vars SUPABASE_URL, SUPABASE_ANON_KEY (optional fallback to local JSON) out:upserted records (snake_case), search run logs, exclusion list err:SupabaseError, missing env vars, partial batch failures (silent)
