// @crumb frontend-component-custom-kpi-modal
// UI/Analytics/Manager | kpi_builder_form | metric_target_period | supabase_upsert | on_saved_callback
// why: Custom KPI modal — create or edit a custom KPI definition for Manager-role dashboards, including metric type, target value, and display preferences
// in:kpi (optional for edit),useAuth (org_id),supabase kpis table out:New or updated KPI row,onSaved called err:Supabase upsert failure,missing org_id (DB constraint fail)
// hazard: KPI targets stored as bare numeric without unit — display context inferred from metric type, fragile for new types
// hazard: No RBAC enforcement in this component — non-Manager roles can create/modify KPIs if modal is rendered
// edge:frontend/src/hooks/useAuth.ts -> READS
// edge:custom-kpi#1 -> STEP_IN
// prompt: Add unit field to KPI schema. Enforce Manager role check before rendering this modal. Support formula-based KPIs.
